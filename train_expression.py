...
class Normalize(object):   # data normalization between -1 and 1 and then converting to Tensor
    def __call__(self, sample):
        noisy_param, label_param = sample['noisy_param'], sample['label_param']
        mins = np.amin(noisy_param, axis=0)
        maxs = np.amax(noisy_param, axis=0)
        noisy_param = 2*(noisy_param - mins)/(maxs - mins) - 1
        label_param = 2*(label_param-mins)/(maxs - mins) - 1
        return {'noisy_param': torch.from_numpy(noisy_param).float(),
                'label_param': torch.from_numpy(label_param).float()}
...
batch_size = 512
learning_rate = 0.001
data_path = 'dataset_generation/dataset_expression/'
input_length = 29
dataset = Noisy3dmmDataset(data_path + 'train/', transform=ToTensor())
...
save_folder = './trained_models/expression/'
